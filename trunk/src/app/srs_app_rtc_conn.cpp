...
{
  srs_error_t err = srs_success;
  for (std::map<uint32_t, SrsRtcVideoSendTrack*>::iterator iter =
           video_tracks_.begin();
       iter != video_tracks_.end(); iter++) {
    SrsRtcVideoSendTrack* track = iter->second;
    if ((err = track->send_rtcp_sr(now_ms)) != srs_success) {
      return srs_error_wrap(err, "video send rtcp sr error track=%s",
                            track->get_track_id().c_str());
    }
  }

  for (std::map<uint32_t, SrsRtcAudioSendTrack*>::iterator iter =
           audio_tracks_.begin();
       iter != audio_tracks_.end(); iter++) {
    SrsRtcAudioSendTrack* track = iter->second;
    if ((err = track->send_rtcp_sr(now_ms)) != srs_success) {
      return srs_error_wrap(err, "audiosend rtcp sr error track=%s",
                            track->get_track_id().c_str());
    }
  }
...
srs_error_t SrsRtcPlayStream::on_rtcp_rr(SrsRtcpRR* rtcp, srs_utime_t now_ms)
{
    ...
    for (std::vector<SrsRtcpRB>::iterator iter = rtcp->rr_blocks_.begin();
         iter != rtcp->rr_blocks_.end(); iter++) {
      SrsRtcpRB& rb = *iter;
      uint32_t ssrc = rb.ssrc;

      for (std::map<uint32_t, SrsRtcAudioSendTrack*>::iterator audio_iter =
               audio_tracks_.begin();
           audio_iter != audio_tracks_.end(); audio_iter++) {
        if (ssrc == audio_iter->second->track_desc_->ssrc_) {
          return audio_iter->second->handle_rtcp_rr(rb, now_ms);
        }
      }

      for (std::map<uint32_t, SrsRtcVideoSendTrack*>::iterator video_iter =
               video_tracks_.begin();
           video_iter != video_tracks_.end(); video_iter++) {
